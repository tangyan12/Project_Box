...
unsigned char My_card_1[4]= {211,249,18,53};//我自己的卡号
...
card_type MyRC522_Orignize(void)
{
    status=PcdRequest(REQ_ALL,CT);
    if (status==MI_OK)
    {
        u3_printf("有卡片进入检测范围");
        status=MI_ERR;
        ...
        if (status==MI_OK)
        {
            if (SN[0]==My_card_1[0] && SN[1]==My_card_1[1]\
               && SN[2]==My_card_1[2] && SN[3]==My_card_1[3])
            {
                u3_printf("检测到已注册的用户卡");
                return ID_card_1;
            }   
        }
    }
